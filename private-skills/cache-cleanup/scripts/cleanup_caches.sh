  ...
  case $cache_type in
    mise)
      if [[ -d ~/.cache/mise ]]; then
        ...
        # Check for corrupted npm backend cache
        local corrupted_msgpack=$(find ~/.cache/mise/npm-* -name "*.msgpack.z" -size -100c 2>/dev/null | wc -l)
        if [[ $corrupted_msgpack -gt 0 ]]; then
          log_warning "  Corrupted npm backend cache files: $corrupted_msgpack"
          echo "  (These small msgpack.z files are likely corrupted)"
        fi
      else
...
cleanup_cache() {
  ...
  case $cache_type in
    mise)
      ...
      # Clean npm backend cache (can become corrupted due to race conditions)
      rm -rf ~/.cache/mise/npm-* 2>/dev/null || true
      log_success "mise cache cleaned (including npm backend)"
      ;;
    npm)
