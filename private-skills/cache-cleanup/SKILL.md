...
rm -rf ~/.cache/mise/npm-* # Clean npm backend cache (can become corrupted)
```

**Known Issue - npm Backend Cache Corruption**:
- The npm backend (`npm:` prefixed tools) stores cache in `~/.cache/mise/npm-*`
- Concurrent `mise upgrade` with `jobs > 1` can cause race conditions
- Corrupted `.msgpack.z` files cause warnings like: `failed to parse cache file... failed to fill whole buffer`
- **Solution**: Clean npm backend cache before upgrade: `rm -rf ~/.cache/mise/npm-*`

**Pre-check reports**:
- Cache directory size
- Number of installed tools
- Old versions that can be pruned
- Corrupted npm backend cache files (small `.msgpack.z` files)
