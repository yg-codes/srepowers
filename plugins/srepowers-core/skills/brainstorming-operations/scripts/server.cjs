...
const SREPOWERS_VERSION = readSrepowersVersion();
...
function readSrepowersVersion() {
  // Scripts live at plugins/srepowers-core/skills/brainstorming-operations/scripts,
  // so ../../.. resolves to the plugin root (plugins/srepowers-core) and its
  // parent chain reaches the repo root. Try the plugin manifest first, then the
  // repo-root Codex plugin manifest for packaged installs.
  ...
  for (const manifest of manifests) {
    try {
      ...
      // Packaged plugins may omit some manifests; try the next one.
    }
  }
...
function brandMarkup() {
  const version = escapeHtmlText(SREPOWERS_VERSION);
  const text = 'SREPowers v' + version;
  return '<div class="brand"><a href="https://github.com/yg-codes/srepowers"><span class="brand-copy">' + text + '</span></a></div>';
}
