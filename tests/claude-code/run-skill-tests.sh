...
while [[ $# -gt 0 ]]; do
	...
	--help | -h)
		...
		echo "    test-finishing-operation-branch.sh      Branch completion workflow"
		echo "    test-using-git-worktrees-sre.sh         Worktree isolation"
		echo "    test-verification-before-completion.sh  Evidence-before-claims gate"
		echo "    test-writing-skills-sre.sh              Standalone SRE skill authoring"
		echo "  Infrastructure:"
		...
		echo "    test-environment-health-check.sh        Tool and context diagnostics"
		echo "    test-incident-commander.sh              ICS-style incident coordination"
		echo "    test-progressive-delivery.sh            Canary and blue-green workflows"
		echo "    test-safety-validator.sh                Risk classification and safer alternatives"
		echo "    test-systematic-troubleshooting.sh      4-phase root cause analysis"
		echo "    test-find-polluter.sh                   find-polluter glob matching and counts"
		...
		echo "    test-container-engineer.sh              Container builds and security"
		...
		echo "    test-observability-engineer.sh          Observability stack and alerting"
		echo "    test-postgresql-engineer.sh             PostgreSQL operations and SQL"
		echo "    test-postgresql-engineer-sql.sh         SQL optimization and indexing"
...
tests=(
	...
	"test-using-srepowers-routing.sh"
	"test-evidence-first-reporting.sh"
	"test-writing-skills-sre.sh"
	"test-sre-runbook.sh"
	"test-pve-admin.sh"
	"test-puppet-code-analyzer.sh"
	"test-gitlab-ecr-pipeline.sh"
	"test-environment-health-check.sh"
	"test-incident-commander.sh"
	"test-progressive-delivery.sh"
	"test-safety-validator.sh"
	...
	"test-observability-engineer.sh"
	"test-postgresql-engineer.sh"
	"test-postgresql-engineer-sql.sh"
	"test-python-pro.sh"
